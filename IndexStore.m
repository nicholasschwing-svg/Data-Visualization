...
% IndexStore SQLite-backed item index for timeline queries.
    switch lower(action)
        ...
        case 'dirmtime'
            ...
            if isempty(rows), varargout{1} = []; else, varargout{1} = rows{1}; end
        case 'touchdir'
        ...
        case 'queryrange'
            ...
            conn = sqlite(dbPath, 'connect'); c = onCleanup(@() close(conn)); %#ok<NASGU>
            rows = fetch(conn, sprintf(['SELECT item_id,source_id,file_path,timestamp_utc,duration_ms,kind,metadata_json,file_mtime,file_size FROM items ' ...
                'WHERE source_id IN (%s) AND timestamp_utc BETWEEN %d AND %d ORDER BY timestamp_utc'], in, int64(startMs), int64(endMs)));
            varargout{1} = cell2table(rows, 'VariableNames', {'item_id','source_id','file_path','timestamp_utc','duration_ms','kind','metadata_json','file_mtime','file_size'});
        case 'summaries'
            dbPath = varargin{1};
            conn = sqlite(dbPath, 'connect'); c = onCleanup(@() close(conn)); %#ok<NASGU>
            ...
            if isempty(rows)
                ...
                varargout{1} = cell2table(rows, 'VariableNames', {'source_id','label','type','root_path','enabled','item_count','min_ts','max_ts'});
            end
        otherwise
